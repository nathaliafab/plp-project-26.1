...
public class Contexto<T> {
	 ...
	 */
        ...
        protected Stack<java.util.HashSet<Id>> opcionalPilha;
	 ...
	 */
        public Contexto() {
                ...
                opcionalPilha = new Stack<java.util.HashSet<Id>>();
        }

        public void incrementa() {
                pilha.push(new HashMap<Id, T>());
                opcionalPilha.push(new java.util.HashSet<Id>());
        }
        ...
        public boolean isAlreadyOptional(Id idArg) { 
                try { 
                        ...
                        Stack<java.util.HashSet<Id>> auxStack = new Stack<java.util.HashSet<Id>>(); 
                        while (!result && !opcionalPilha.empty()) { 
                                java.util.HashSet<Id> aux = opcionalPilha.pop(); 
                                result = aux.contains(idArg); 
                                auxStack.push(aux); 
                        } 
                        while (!auxStack.empty()) { 
                                opcionalPilha.push(auxStack.pop()); 
                        } 
                        return result; 
                } catch (Exception e) { 
                        return false; 
	 ...
	 */
        public void map(Id idArg, T valorId) throws VariavelJaDeclaradaException {
                map(idArg, valorId, false);
        }
        
        public void map(Id idArg, T valorId, boolean isOptional) throws VariavelJaDeclaradaException {
                try {
                        HashMap<Id, T> aux = pilha.peek();
                        if (aux.containsKey(idArg)) {
                                if (!opcionalPilha.peek().contains(idArg)) {
                                        throw new IdentificadorJaDeclaradoException();
                                }
                        }
                        aux.put(idArg, valorId);
                        if (isOptional) {
                                opcionalPilha.peek().add(idArg);
                        } else {
                                opcionalPilha.peek().remove(idArg);
                        }
